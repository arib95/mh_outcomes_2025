...
# ==================================================
# 2. Master TCP analysis dataframe
# ==================================================
...
tcp_df <- u_df %>%
  ...
  mutate(
    ...
    gender = factor(
      sex,
      levels = c("M", "F", "Male", "Female"),
      labels = c("Male", "Female", "Male", "Female")
    ),
    race = case_when(
      str_detect(race_main, regex("american indian|alaska", ignore_case = TRUE)) ~ "AI/AN",
      str_detect(race_main, regex("asian", ignore_case = TRUE))                 ~ "Asian",
      str_detect(race_main, regex("black|african", ignore_case = TRUE))         ~ "Black",
      str_detect(race_main, regex("white", ignore_case = TRUE))                 ~ "White",
      TRUE                                                                      ~ "Other"
    ),
...
table_quads %>%
  as_flex_table() %>%
  save_as_docx(path = "out/TCP_Table1_Demographics_by_Quadrant.docx")
...
plot_df <- tcp_df %>%
  mutate(
    PlotStatus = factor(
      if_else(NODIAG == 1, "No SCID Diagnosis", "Any Lifetime SCID Diagnosis"),
      levels = c("Any Lifetime SCID Diagnosis", "No SCID Diagnosis")
    )
  )

# Basic QC
nrow(plot_df)
table(is.na(plot_df$PlotStatus))

# Plot origin (centroid)
cm_u1 <- 0
cm_u2 <- 0

p_quadrants <- ggplot(plot_df, aes(x = u1, y = u2, colour = PlotStatus)) +
  annotate(
    "path",
...
save_plot_gg(
  "FIG_TCP_participant_quadrant_map",
  p_quadrants,
  width = 8,
  height = 7.5,
  dpi = 300
