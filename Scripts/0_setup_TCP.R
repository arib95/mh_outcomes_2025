...
COERCE_NUMERIC_01_TO_FACTOR <- TRUE
COERCE_SMALL_INT_NUMERIC_TO_FACTOR <- TRUE
SMALL_INT_NOMINAL_MAX_UNIQUE <- 20L
ITEM_RESPONSE_SCHEMA_CSV <- "data/item_response_schema.csv"
REQUIRE_ITEM_RESPONSE_SCHEMA <- TRUE
INVALID_RESPONSE_ACTION <- "missing"
MAX_PARTICIPANT_MISSING_PROP <- 0.20
PCA_NOMINAL_MISSING <- "mode"
MISSING_AS_NOMINAL_LEVEL  <- FALSE
VALIDATE_ENCODING_ONLY <- FALSE
W_MIN                <- 0.01
...
MM_UNITS      <- FALSE
GRID_N_B      <- 400L
# Base-space convex-hull mask / field resolution; higher values smooth hull edges.
GRID_N_U      <- 400L
...
ok <- try({
  # Rebuild the backend when setup is re-sourced; a failed multisession worker
  # can otherwise leave the existing cluster unusable for the next batch.
